            ...
            if import_file:
                ...
                if file_name.endswith('.zip'):
                    ...
                    if os.path.exists(temp_file_path):
                        ...
                        project = import_file.read_zip(temp_file_path, request.user)  # 返回空间id或None
                        if project:
...
class ImportLocalDoc(APIView):
    ...
    def post(self, request):
        ...
        # pdf 文件
            ...
            temp_file_name = str(time.time()) + '.pdf'
            temp_file_path = os.path.join(settings.MEDIA_ROOT, 'import_temp/' + temp_file_name)
...
def project_doc_sort(request):
    ...
    title = request.POST.get('title', None)  # 空间名称
    desc = request.POST.get('desc', None)  # 空间简介
    role = request.POST.get('role', 1)  # 空间权限
